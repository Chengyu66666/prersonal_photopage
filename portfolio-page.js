 ...
 *
 * Every collection and photo comes from `window.PHOTO_LIBRARY` (generated by
 * `scripts/optimize_images.py`). The grid uses the `thumb` derivative and the
 * lightbox uses `large`, so a page view never pulls a 30 MB camera master.
 *
      ...
      .map(
        (photo, index) => `
          <button class="masonry-photo ${photo.ratio || "standard"}" type="button" data-index="${index}">
            <img src="${gridSource(photo)}" alt="${photo.title}" loading="lazy">
          </button>
        `
      )
      .join("");
